...
const NotificationCard = ({data,index,notificationState}) => {
   let [isReplying,setReplying] = useState(false)
    let {seen,type,reply,createdAt,replied_on_comment,comment,user,user:{personal_info:{profile_img,fullname,username}},blog:{_id,title,blog_id},_id:notification_id} = data
    ...
    <div className={'p-6 border-b border-grey border-l-black '+(!seen ? 'border-l-2 ':"" )}>
      <div className="flex gap-5 mb-3">
        ...
        <div className="w-full">
            <h1 className='font-medium text-xl text-dark-grey'>
                <span className="lg:inline-block capitalize hidden">{fullname}</span>
                <Link to ={`/user/${username}`} className='text-black underline mx-1'> @{username}</Link>
                <span className='font-normal'>
                  {
                    type == 'like' ? "liked your blog":
                    type == 'comment' ? "commented on your blog" : "replied on"
                  }
                </span>
            </h1>
            {
      ...
      }
      {
          ...
          <div className="ml-20 p-5 bg-grey mt-5 rounded-md">
             ...
             <button className='underline hover:text-black ml-14 mt-2' onClick={(e)=>handleDelete(reply,"reply",e.target)}>Delete</button>
          </div>:""
      }
    </div>
  )
